...
TinyMCE_Engine.prototype = {
			...
			if (ar[x] != null && ar[x] != 'null' && ar[x].length > 0) {
				...
				if (!skip) {
					if (tinyMCE.settings.strict_loading_mode) {
						...
						document.write('<link href="../../../scripts/tiny_mce/' + ar[x] + '" rel="stylesheet" type="text/css" />');
	...
	getMenuButtonHTML : function(id, lang, img, mcmd, cmd, ui, val) {
		...
		if (tinyMCE.getParam('button_tile_map') && (!tinyMCE.isIE || tinyMCE.isOpera) && (m = tinyMCE.buttonMap[id]) != null && (tinyMCE.getParam("language") == "en" || img.indexOf('$lang') == -1)) {
			...
			h += '<img src="../../../scripts/tiny_mce/{$themeurl}/images/spacer.gif" style="width: 20px; height: 20px; background-position: ' + x + 'px 0" title="{$' + lang + '}" /></a>';
			h += '<a href="javascript:' + mcmd + '" onclick="' + mcmd + 'return false;" onmousedown="return false;"><img src="../../../scripts/tiny_mce/{$themeurl}/images/button_menu.gif" title="{$' + lang + '}" class="mceMenuButton" />';
			h += '</a></span>';
		} else {
			if (tinyMCE.isRealIE)
			...
			h += '<img src="../../../scripts/tiny_mce/' + img + '" title="{$' + lang + '}" /></a>';
			h += '<a href="javascript:' + mcmd + '" onclick="' + mcmd + 'return false;" onmousedown="return false;"><img src="../../../scripts/tiny_mce/{$themeurl}/images/button_menu.gif" title="{$' + lang + '}" class="mceMenuButton" />';
			h += '</a></span>';
		}
	...
	update : function() {
		...
		for (i=0; i<m.length; i++) {
			...
			switch (m[i].type) {
				...
				default:
					h += '<tr><td><a href="../../../scripts/tiny_mce/' + tinyMCE.xmlEncode(m[i].js) + '" onmousedown="' + tinyMCE.xmlEncode(m[i].js) + ';return tinyMCE.cancelEvent(event);" onclick="return tinyMCE.cancelEvent(event);" onmouseup="return tinyMCE.cancelEvent(event);"><span' + c +'>' + t + '</span></a>';
			}
